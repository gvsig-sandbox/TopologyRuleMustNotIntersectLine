...
class MarkLineAction(AbstractTopologyRuleAction):
    ...
    def execute(self, rule, line, parameters):
        try:
            ...
            for featureReference in theDataSet.query(geometry1):
                feature2 = featureReference.getFeature()
                line2 = feature2.getDefaultGeometry()
                if not geometry1.equals(line2) and geometry1.intersects(line2):
                    intersection = geometry1.intersection(line2)
                    geometryType1 = intersection.getGeometryType()
                    if geometryType1.getType() == geom.LINE or geometryType1.isTypeOf(geom.LINE):
                        features = errorsLayer.features()
                        flag = False
                        for feature in features:
                            if feature.geometry().equals(intersection):
                                flag = True
                                break
                        if not flag:
    ...
    def checkErrorsLayerName(self):
        if self.errorsLayerName == "":
            self.errorsLayerName = "MustNotIntersect_" + datetime.now().strftime("%Y%m%d%H%M%S")
